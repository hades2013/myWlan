...
int qca6174TxCommand(const char *txtype)
{
	...
	if (gCmd.broadcast != 1){
		addParameterToCommand((uint8_t *)"broadcast",(uint8_t *)&gCmd.broadcast, sizeof(gCmd.broadcast));
	...
	if (is_zero_ether_addr(gCmd.bssid)) {
		...
		addParameterToCommand((uint8_t *)"bssid",(uint8_t *)mac, sizeof(mac));
		ALOGE("\n%s() adding command(bssid, %x:%x:%x:%x:%x:%x)\n", __func__, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
	}
	else {
		addParameterToCommand((uint8_t *)"bssid",(uint8_t *)gCmd.bssid, sizeof(gCmd.bssid));
	...
	if (is_zero_ether_addr(gCmd.txStation)) {
		...
		addParameterToCommand((uint8_t *)"txStation",(uint8_t *)mac,sizeof(mac));
		ALOGE("\n%s() adding command(txStation, %x:%x:%x:%x:%x:%x)\n", __func__, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
	}
	else{
		addParameterToCommand((uint8_t *)"txStation",(uint8_t *)gCmd.txStation, sizeof(gCmd.txStation));
	...
	if (is_zero_ether_addr(gCmd.rxStation)) {
		...
		addParameterToCommand((uint8_t *)"rxStation",(uint8_t *)mac, sizeof(mac));
		ALOGE("\n%s() adding command(rxStation, %x:%x:%x:%x:%x:%x)\n", __func__, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

	}
	else{
		addParameterToCommand((uint8_t *)"rxStation",(uint8_t *)gCmd.rxStation, sizeof(gCmd.rxStation));
...
//	addParameterToCommand((uint8_t *)"tpcm",(uint8_t *)&gCmd.tpcm);
	addParameterToCommand((uint8_t *)"flags",(uint8_t *)&miscFlags, sizeof(miscFlags));
      ALOGE("\n%s() adding command(flags, %d)\n", __func__, miscFlags);

	if (wlanMode != TCMD_WLAN_MODE_HT20){
		addParameterToCommand((uint8_t *)"wlanMode",(uint8_t *)&wlanMode, sizeof(wlanMode));
	...
	addParameterToCommand((uint8_t *)"txMode",(uint8_t *)&txMode, sizeof(txMode));
	ALOGE("\n%s() adding command(txMode, %d)\n", __func__, txMode);

	value = PN9_PATTERN;
	addParameterToCommand((uint8_t *)"txPattern",(uint8_t *)&value, sizeof(value));
	ALOGE("\n%s() adding command(txPattern, %d)\n", __func__, value);

	if (gCmd.aifs){
		addParameterToCommand((uint8_t *)"aifsn",(uint8_t *)&gCmd.aifs, sizeof(gCmd.aifs));
...
#endif
	addParameterToCommand((uint8_t *)"rateBitIndex0",(uint8_t *)&value, sizeof(value));
      printf("dataRate %d rateIndex %d \n", dataRate, value);

	if (gCmd.shortguard) {
		value = 1;
		addParameterToCommand((uint8_t *)"shortGuard",(uint8_t *)&value, sizeof(value));
		ALOGE("\n%s() adding command(shortGuard, %d)\n", __func__, value);
	}

	if (!gCmd.pktLen0)
		gCmd.pktLen0 = 1500;
	addParameterToCommand((uint8_t *)"pktLen0",(uint8_t *)&gCmd.pktLen0, sizeof(gCmd.pktLen0));
	ALOGE("\n%s() adding command(pktLen0, %d)\n", __func__, gCmd.pktLen0);
	addParameterToCommand((uint8_t *)"txChain0",(uint8_t *)&gCmd.chain, sizeof(gCmd.chain));
	ALOGE("\n%s() adding command(txChain0, %d)\n", __func__, gCmd.chain);

	value = 1;
	addParameterToCommand((uint8_t *)"ir",(uint8_t *)&value, sizeof(value));
	ALOGE("\n%s() adding command(ir, %d)\n", __func__, value);

	value = gCmd.txPwr * 2;
	addParameterToCommand((uint8_t *)"txPower0",(uint8_t *)&value, sizeof(value));
	ALOGE("\n%s() adding command(txPower0, %d)\n", __func__, value);

	value = 0;
	addParameterToCommand((uint8_t *)"paConfig",(uint8_t *)&value, sizeof(value));
...
void addRateMaskToCommand(int dataRate)
{
	...
	if ( dataRate == 153 ) // Sweep all rates
	{
		...
		addParameterToCommand((uint8_t *)rateMask,(uint8_t *)&rateBitMask, sizeof(rateBitMask));
		...
		if ( rowIndex != 0 ) {
			...
			snprintf((char*)rateMask,25,"%s%d","rateMask",0);
			value = 0;
			addParameterToCommand((uint8_t *)rateMask,(uint8_t *)&value, sizeof(value));
		}
	}
}
...
int qca6174RxPacketStart(void)
{
	...
	if (gCmd.freq != 2412)
		addParameterToCommand((uint8_t *)"channel",(uint8_t *)&gCmd.freq, sizeof(gCmd.freq));

	if (gCmd.antenna != 0)
		addParameterToCommand((uint8_t *)"antenna",(uint8_t *)&gCmd.antenna, sizeof(gCmd.antenna));

	if (gCmd.ani != 0)
		addParameterToCommand((uint8_t *)"enANI",(uint8_t *)&gCmd.ani, sizeof(gCmd.ani));

	addParameterToCommand((uint8_t *)"wlanMode",(uint8_t *)&wlanMode, sizeof(wlanMode));
	addParameterToCommand((uint8_t *)"rxChain",(uint8_t *)&gCmd.chain, sizeof(gCmd.chain));
	...
	if (is_zero_ether_addr(gCmd.addr)) {
		...
		addParameterToCommand((uint8_t *)"addr",(uint8_t *)mac, sizeof(mac));
	}
	else
		addParameterToCommand((uint8_t *)"addr",(uint8_t *)gCmd.addr, sizeof(gCmd.addr));

	addParameterToCommand((uint8_t *)"expectedPkts",(uint8_t *)&gCmd.expectedPkts, sizeof(gCmd.expectedPkts));
	addParameterToCommand((uint8_t *)"flags",(uint8_t *)&miscFlags, sizeof(miscFlags));
...
int qca6174RxPacketStop(void)
{
	...
	if (1 || gCmd.isRxStart)
	{
		...
		value = 0xb7;
		addParameterToCommand((uint8_t *)"commandId",(uint8_t *)&value, sizeof(value));

		value = 1;
		addParameterToCommand((uint8_t *)"param1",(uint8_t *)&value, sizeof(value));
