...
boolean ftm_bt_hci_hal_read_soc_version()
{
  const uint8 getAppVerCmd[] = {0x19};
  global_state = FTM_SOC_READ_APP_VER;
  return ftm_bt_hci_hal_vs_sendcmd(BT_QSOC_EDL_CMD_OPCODE,(uint8 *)&getAppVerCmd[0],1);
}
...
boolean ftm_bt_hal_soc_init()
{
  ...
  printf("%s: Check if BT is already ON\n", __func__);
  if(ftm_bt_power_hal_check() != BT_ON)
  {
     printf("%s: BT is OFF: Turning BT ON...\n", __func__);
     ret = ftm_bt_power_hal_set(BT_ON);
     if(ret != STATUS_SUCCESS)
        return FALSE;
     printf("%s: ***** BT-ON SUCCESS *****\n", __func__);
  }
  else
  {
    printf("%s: ##### W A R N I N G ##### : BT is ON probably!!!\n", __func__);
  }

  printf("%s: Initializing the UART Transport\n", __func__);
  ret = ftm_bt_hci_hal_init_transport() ;
  if(ret != STATUS_SUCCESS)
  {
	printf("%s: Failed to init transport\n", __func__);
    return FALSE;
  }
  printf("Transport open ret = %d\n",ret);

  printf("%s: Read ROME Chip version\n", __func__);
  ftm_bt_hci_hal_read_soc_version();
  return TRUE;
}
...
void ftm_bt_dispatch(void *ftm_bt_pkt ,int cmd_len )
{
  ...
  printf("%s: Calling ftm_bt_hci_send_cmd()", __func__); 
...
)
{
  ...
  printf("%s: Global-State: %d\n", __func__, global_state);
  if(NULL == cmd_buf)
  {
    return FALSE;
  }
#ifdef BT_NV_SUPPORT
  printf("%s: BT_NV_SUPPORTED\n", __func__);
  ...
  printf("%s: cmd_buf[0]: %d\n", __func__, *cmd_buf);
  if (*cmd_buf == FTM_BT_DRV_START_TEST)
  {
    printf("%s: FTM_BT_DRV_START_TEST\n", __func__);
  ...
  if(global_state == FTM_SOC_NOT_INITIALISED)
  {
    ...
    printf("%s: FTM_SOC_NOT_INITIALISED\n", __func__);
    ...
    printf("%s: SOC-HAL Initialization...\n", __func__);
...
boolean ftm_bt_hci_qcomm_handle_event ()
{
  ...
  switch(global_state)
  {
    case FTM_SOC_READ_APP_VER:
      ...
      printf("%s: Sending HCI RESET to ROME-Controller\n", __func__);
