...
boolean ftm_bt_hci_pfal_set_transport(void)
{
    ...
    if(ret == 0)
        printf("%s: ro.qualcomm.bt.hci_transport not set\n", __func__);
    else
        printf("%s: ro.qualcomm.bt.hci_transport: %s \n", __func__, transport_type);
...
#ifdef ANDROID
        property_set("bluetooth.FTM_MODE","1");
#endif
...
request_status ftm_bt_hci_pfal_init_transport()
{
  ...
  printf("%s: Opening handle to TTY Serial driver\n", __func__);
    ...
    printf("%s: Enabling UART HW FW CONTROL\n", __func__);
  ...
  printf("%s: Creating the Event Listener thread\n", __func__);
...
request_status ftm_bt_hci_pfal_nwrite(uint8 *buf, int size)
{
  ...
  if (verbose == 1)
  {
    printf("[%s] %s: BT CMD:", __TIME__, __FUNCTION__);
