...
)
{
  ...
  for(i = 0; i < nparams; i++)
  {
    ...
    strlcpy((char *)(&cmd[NV_CMD_HDR_SIZE]), (const char *)(params[i].pCmdBuffer), NV_BD_ADDR_LEN);
                ...
                nv_persist_item_type *my_nv_item, int bIsRandom)
{
  ...
  /* 'numitems' valid range: NV_BT_ITEM_MIN to (NV_BT_ITEM_MAX - 1) */
  numitems = (numitems >= NV_BT_ITEM_MAX) ? (NV_BT_ITEM_MAX -1 ) : numitems;

  /* 'nvitem' valid range: NV_BT_ITEM_MIN < nvitem < NV_BT_ITEM_MAX */
  ...
  switch(nvReadWriteFunc)
  {
    case NV_READ_F: //read from file
      ...
      {
        strlcpy((char *)((*my_nv_item).bd_addr), (const char *)(params_temp[item].pCmdBuffer), NV_BD_ADDR_LEN );
