...
)
{
  ...
  control.value = (int)(&poke_word.data);
  err = ioctl(fd_radio,VIDIOC_S_CTRL,&control);
  if(err >= 0)
  {
...
void ftm_fm_run_mm(void) {
   ...
   if (!pid) {
       ...
       ALOGE("mm module error %d\n", err);
       exit(0);
   } else if (pid < 0) {
       ALOGE("could not create child process to execute mm audio module\n");
   }
   return;
}
...
fm_cmd_status_type ftm_fm_audio(uint8 source, uint8 volume) {
   ...
   ALOGE("string = %s \n",cmd);
   ALOGE("audio_output = %d\n", source);
   if (source) {
       if (strlcat(cmd, "51 -v ", CMD_len) >= CMD_len) {
           free(cmd);
           return FM_CMD_FAILURE;
       }
   } else {
       if (strlcat(cmd, "50 -v ", CMD_len) >= CMD_len) {
           free(cmd);
           return FM_CMD_FAILURE;
       }
   }
   ALOGE(" string = %s \n",cmd);
   if (snprintf(vlm, sizeof vlm, "%d", volume) >=  sizeof vlm) {
       free(cmd);
       return FM_CMD_FAILURE;
   }
   ...
   if (ftm_audio_fd < 0) {
       ALOGE("Failed to open ftm_commands with write\n");
       free(cmd);
       return FM_CMD_FAILURE;
   }
   ALOGE(" writing config path\n");
   if (write(ftm_audio_fd, audio_config, config_len) != config_len) {
       ALOGE("Failed to write config_file\n");
       free(cmd);
       close(ftm_audio_fd);
       return FM_CMD_FAILURE;
   }
   ALOGE("done write config path\n");
   sleep(1);
   ALOGE("writing command for path = %d \n", strlen(cmd));
   if (write(ftm_audio_fd, cmd, strlen(cmd) + 1) != (strlen(cmd) + 1)) {
       ALOGE("Failed to write config_file\n");
       free(cmd);
       close(ftm_audio_fd);
       return FM_CMD_FAILURE;
   }
   ALOGE("done write command for path\n");
   free(cmd);
   return FM_CMD_SUCCESS;
}
