  ...
  if(cmd_len >1)
  {
    switch(*(cmd_buf+1))
    {
      ...
      case NV_BT_SOC_REFCLOCK_TYPE_I:
        cmd_result = (nv_persist_stat_enum_type)bt_nv_cmd(NV_READ_F,  NV_BT_SOC_REFCLOCK_TYPE_I, &my_nv_item, 0);
        if (NV_SUCCESS != cmd_result)
        {
          if (verbose > 0)
          {
      ...
      case NV_BT_SOC_CLK_SHARING_TYPE_I:
        cmd_result = (nv_persist_stat_enum_type)bt_nv_cmd(NV_READ_F,  NV_BT_SOC_CLK_SHARING_TYPE_I, &my_nv_item, 0);
        if (NV_SUCCESS != cmd_result)
        {
          if (verbose > 0)
          {
  ...
  if(cmd_len >1)
  {
    switch(*(cmd_buf+1))
    {
      case NV_BD_ADDR_I:
        ...
        cmd_result = (nv_persist_stat_enum_type)bt_nv_cmd(NV_WRITE_F,  NV_BD_ADDR_I, &my_nv_item, 0);
        if (NV_SUCCESS != cmd_result)
        {
          if (verbose > 0)
          {
      ...
      case NV_BT_SOC_REFCLOCK_TYPE_I:
        ...
        if (result != FALSE)
        {
          cmd_result= (nv_persist_stat_enum_type)bt_nv_cmd(NV_WRITE_F,  NV_BT_SOC_REFCLOCK_TYPE_I, &my_nv_item, 0);
          if (NV_SUCCESS != cmd_result)
          {
      ...
      case NV_BT_SOC_CLK_SHARING_TYPE_I:
        ...
        if (result != FALSE)
        {
          cmd_result= (nv_persist_stat_enum_type)bt_nv_cmd(NV_WRITE_F, NV_BT_SOC_CLK_SHARING_TYPE_I, &my_nv_item, 0);
          if (NV_SUCCESS != cmd_result)
          {
