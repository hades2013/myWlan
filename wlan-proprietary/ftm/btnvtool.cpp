...
int main(int argc, char *argv[])
{
  ...
  memset(&my_nv_item, 0, sizeof(my_nv_item));
  while (TRUE)
  {
    ...
    switch (c)
    {
       case 'b':
         ...
         strlcpy((char *)(&my_nv_item.bd_addr), (const char *)BD_addr, NV_BD_ADDR_LEN);
         bt_nv_cmd(NV_WRITE_F,  NV_BD_ADDR_I, &my_nv_item, 0);
         break;
       ...
       case 'r':
         ...
         bt_nv_cmd(NV_WRITE_F,  NV_BT_SOC_REFCLOCK_TYPE_I, &my_nv_item, 0);
         break;
       ...
       case 'c':
         ...
         bt_nv_cmd(NV_WRITE_F, NV_BT_SOC_CLK_SHARING_TYPE_I, &my_nv_item, 0);
         break;

       case 'p':
         if (bt_nv_cmd(NV_READ_F, NV_BD_ADDR_I, &my_nv_item, 0) != 0)
             fprintf(stderr, "Failed to read NV Items from persist!\n");
         else {
         ...
         bt_nv_cmd(NV_READ_F, NV_BT_SOC_REFCLOCK_TYPE_I, &my_nv_item, 0);
         fprintf (stderr, "--reference-clock: %s MHz \n",
                     my_nv_item.bt_soc_refclock_type?"19.2":"32" );
         bt_nv_cmd(NV_READ_F, NV_BT_SOC_CLK_SHARING_TYPE_I, &my_nv_item, 0);
