...
void lim_check_sta_in_pe_entries(tpAniSirGlobal pMac, tpSirMacMgmtHdr pHdr)
{
	...
	#if 1 //wewins
		deauth_mac.del_sta = 1;
		memcpy(deauth_mac.mac, (tANI_U8 *) pHdr->sa,sizeof(deauth_mac.mac));
	#endif
                break;
            }
        }
...
#if 1  //wewins
	if(deauth_mac.del_sta == 1 && memcmp(deauth_mac.mac,pHdr->sa,sizeof(deauth_mac.mac)) == 0 )
	{
		limLog(pMac, LOGE, FL("deauth direct ignore "MAC_ADDRESS_STR),MAC_ADDR_ARRAY(pHdr->sa));
		deauth_mac.del_sta = 0;
		memset(deauth_mac.mac, 0,sizeof(deauth_mac.mac));
		return;
	}
#endif
    /**
